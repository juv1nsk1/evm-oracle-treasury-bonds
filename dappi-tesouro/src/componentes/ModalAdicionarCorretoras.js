...
const ModalAdicionarCorretoras = ({ contrato, setModal }) => {
  ...
  async function EnviarDados(pacote) {
    try {
      if (codigo && endereco) {
        console.log("Adicionando corretora:" , codigo, endereco);
        await contrato.adicionarCorretora(codigo, endereco, { gasLimit: 30000000 });
      } 
    } catch (err) {
    ...
    <div className="col-lg-12 col-sm-12" id="collapseAlterar">
      <div className="card bg-dark p-4 d-flex h-100">
        <div className="card-title">
          <h4 className="title">Cadastrar corretora</h4>
        </div>
        <div className="card-body">
          ...
          <font className="acao">Endereço da Carteira Digital da corretora:</font>
          <div className="form-floating">
            <input type="text" className="form-control" id="floatingInput2" placeholder="Endereço da Corretora" onChange={(e) => setEndereco(e.target.value)} />
            <label htmlFor="floatingInput2">Endereço</label>
          </div>
